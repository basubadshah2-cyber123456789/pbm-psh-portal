...
export async function POST(request: Request) {
  try {
    ...
    const capacitySetting = await prisma.systemSetting.findUnique({
      where: { key: 'MAX_CHILDREN_CAPACITY' },
      select: { value: true },
    });
    const maxChildren = Number.parseInt(capacitySetting?.value || '100', 10);
    const activeChildren = await prisma.child.count({ where: { status: 'ACTIVE' } });
    if (Number.isFinite(maxChildren) && activeChildren >= maxChildren) {
      return NextResponse.json(
        { error: `Active child capacity reached (${maxChildren}). No additional admissions are available.` },
        { status: 400 }
      );
    }
