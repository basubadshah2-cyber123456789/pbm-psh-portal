...
export async function PATCH(request: Request) {
  try {
    ...
    const leave = await prisma.leaveRequest.update({ where: { id }, data: { status, approverId: user.employeeId || null, decisionNote: body.decisionNote ? String(body.decisionNote).trim() : null }, include: { employee: { select: { fullName: true, phoneNumber: true } } } });
    ...
    try { await sendSms(leave.employee.phoneNumber, `Sweet Home Multan: Your ${leave.leaveType} leave request has been ${status.toLowerCase()}.`); } catch (smsError) { console.warn('Leave SMS notification failed:', smsError); }
