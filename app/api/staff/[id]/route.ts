...
import { WEEK_DAYS } from '@/lib/weekly-off';
...
export async function PUT(
  ...
  try {
    ...
    if (weeklyOffDays !== undefined && (!Array.isArray(weeklyOffDays) || weeklyOffDays.some((day) => !WEEK_DAYS.includes(day)))) {
      return NextResponse.json({ error: 'Weekly off days must be valid weekdays' }, { status: 400 });
    }
    ...
    const updatedEmployee = await prisma.employee.update({
      ...
      data: {
        ...
        weeklyOffDays: weeklyOffDays !== undefined ? JSON.stringify(weeklyOffDays) : existingEmp.weeklyOffDays,
      },
    });
