...
export function SettingsManagement() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (logsData.success) setSmsLogs(logsData.logs || []);
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="flex items-center gap-2 px-6 py-4 border-b border-slate-100">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-900">SMS Notification History</h2>
            <p className="text-[11px] text-slate-500">Weekly-off delivery attempts and provider results</p>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-slate-50 text-left text-slate-500">
              <tr><th className="px-6 py-3">Date</th><th className="px-4 py-3">Employee</th><th className="px-4 py-3">Phone</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Details</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {smsLogs.map((log) => (
                <tr key={log.id}>
                  <td className="px-6 py-3 text-slate-600">{new Date(log.notificationDate).toLocaleDateString()}</td>
                  <td className="px-4 py-3 font-semibold text-slate-800">{log.employeeName}</td>
                  <td className="px-4 py-3 text-slate-600">{log.phoneNumber || 'No phone'}</td>
                  <td className="px-4 py-3 text-slate-600">{log.notificationType}</td>
                  <td className="px-4 py-3"><span className={`font-bold ${log.status === 'SENT' ? 'text-emerald-700' : log.status === 'SKIPPED' ? 'text-amber-700' : 'text-red-700'}`}>{log.status}</span></td>
                  <td className="px-4 py-3 text-slate-500">{log.error || '-'}</td>
                </tr>
              ))}
              {!smsLogs.length && <tr><td colSpan={6} className="px-6 py-8 text-center text-slate-400">No SMS history yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
