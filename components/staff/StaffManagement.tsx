...
export function StaffManagement() {
  ...
  const [directoryEmployees, setDirectoryEmployees] = useState<EmployeeDirectoryItem[]>([]);
  ...
  const fetchStaff = async () => {
    ...
    try {
      ...
      const directoryRes = await fetch('/api/employee-data');
      const directoryData = await directoryRes.json();
      if (directoryData.success) setDirectoryEmployees(directoryData.employees);
    } catch (err) {
  ...
  const handleOpenEdit = (emp: EmployeeItem) => {
    ...
    setEditFormData({
      ...
      notes: emp.notes || '',
      newPassword: '',
      weeklyOffDays: JSON.parse(emp.weeklyOffDays || '[]'),
    });
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100">
          <h2 className="text-sm font-bold text-slate-900">Employee Data Directory</h2>
          <p className="text-[11px] text-slate-500 mt-0.5">Employee records ready for remaining information</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {directoryEmployees.map((employee) => (
                <tr key={employee.id} className="hover:bg-slate-50/80">
                  ...
                  <td className="px-4 py-3 text-slate-700">{employee.fatherHusbandName || 'To be added'}</td>
                  <td className="px-4 py-3"><div className="font-mono text-slate-700">{employee.idCardNo}</div><div className="text-[11px] text-slate-500">{employee.phoneNumber || 'Phone to be added'}</div></td>
                  <td className="px-4 py-3 text-emerald-800">{employee.designation}<div className="text-[11px] text-slate-400">Department to be added</div></td>
                  <td className="px-4 py-3 text-slate-500">{employee.loginAccount || 'Not created'}</td>
                  <td className="px-4 py-3"><span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-800">{employee.status}</span></td>
                  <td className="px-4 py-3 text-right text-slate-400">To be updated</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto animate-scale-in">
            ...
            <form onSubmit={handleCreateStaff} className="mt-4 space-y-4 text-xs">
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-2">Weekly Off Days</label>
                <div className="flex flex-wrap gap-2">
                  {weekDays.map((day) => (
                    <label key={day} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-slate-50 border border-slate-200">
                      <input type="checkbox" checked={formData.weeklyOffDays.includes(day)} onChange={(e) => setFormData({ ...formData, weeklyOffDays: e.target.checked ? [...formData.weeklyOffDays, day] : formData.weeklyOffDays.filter((item) => item !== day) })} />
            ...
            <form onSubmit={handleUpdateStaff} className="mt-4 space-y-3 text-xs">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {weekDays.map((day) => (
                    <label key={day} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-slate-50 border border-slate-200">
                      <input type="checkbox" checked={editFormData.weeklyOffDays.includes(day)} onChange={(e) => setEditFormData({ ...editFormData, weeklyOffDays: e.target.checked ? [...editFormData.weeklyOffDays, day] : editFormData.weeklyOffDays.filter((item) => item !== day) })} />
                      {day.charAt(0) + day.slice(1).toLowerCase()}
                    </label>
                  ))}
                </div>
