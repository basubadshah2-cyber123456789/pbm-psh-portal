...
### STEP 8: Run Prisma Migrations Safely
The Vercel production build applies only pending checked-in migrations against the configured PostgreSQL `DATABASE_URL`:
```bash
npx prisma migrate deploy
```
Do not use `prisma db push` or `prisma migrate reset` in production. Existing data is preserved.

### STEP 9: Deploy
Click **Deploy** in Vercel or trigger a deployment from your git branch.
Vercel will execute:
1. `npm install` (triggers `postinstall: prisma generate`)
2. `npm run build` (`prisma migrate deploy && prisma generate && next build`)
...
The weekly-off scheduler calls a replaceable server-side HTTP SMS gateway, suitable for an Android device containing a dedicated Pakistani SIM. Configure `SMS_GATEWAY_ENABLED`, `SMS_GATEWAY_URL`, `SMS_GATEWAY_API_KEY`, `SMS_GATEWAY_DEVICE_ID`, and `SMS_GATEWAY_TIMEOUT_MS` only on the server. The gateway contract accepts `to`, `message`, `externalReference`, and optional `deviceId`, and returns `providerMessageId` or `id` on success.

Use the authenticated cron endpoint with `?dryRun=1` to preview normalized destinations and messages without sending or creating fake successful logs. Real delivery requires both administrative SMS settings enabled and `SMS_GATEWAY_ENABLED=true`. Duplicate protection uses one unique weekly-off log per employee, date, and notification type.
