...
async function main() {
  ...
  for (const entry of employeeDirectoryData) {
    await prisma.employeeDirectoryEntry.upsert({
      where: { srNo: entry.srNo },
      update: entry,
      create: entry,
    });
  }
  console.log(`✅ Seeded ${employeeDirectoryData.length} employee directory records without login accounts`);
  ...
  const buildingsData = [
    {
      ...
      rooms: [
        { roomNumber: 'Room 101', floor: 'Quaid-1', capacity: 10 },
        { roomNumber: 'Room 102', floor: 'Quaid-1', capacity: 15 },
        { roomNumber: 'Room 103', floor: 'Jinnah-2', capacity: 8 },
        { roomNumber: 'Room 104', floor: 'Jinnah-2', capacity: 8 },
        { roomNumber: 'Room 105', floor: 'Jinnah-2', capacity: 9 },
      ],
    },
    {
      ...
      rooms: [
        { roomNumber: 'Room 201', floor: 'Iqbal-3', capacity: 10 },
        { roomNumber: 'Room 202', floor: 'Iqbal-3', capacity: 15 },
        { roomNumber: 'Room 203', floor: 'Ghazali-4', capacity: 8 },
        { roomNumber: 'Room 204', floor: 'Ghazali-4', capacity: 8 },
        { roomNumber: 'Room 205', floor: 'Ghazali-4', capacity: 9 },
      ],
    },
  ];
  ...
  for (const bldg of buildingsData) {
    ...
    for (const r of bldg.rooms) {
      ...
      } else if (room.capacity !== r.capacity) {
        room = await prisma.room.update({
          where: { id: room.id },
          data: { capacity: r.capacity, floor: r.floor },
        });
      }
  ...
  const settingsData = [
    ...
    { key: 'MAX_CHILDREN_CAPACITY', value: '100', group: 'INSTITUTION', description: 'Maximum number of active children the facility can accommodate' },
    { key: 'TOTAL_BED_CAPACITY', value: '100', group: 'INSTITUTION', description: 'Total residential beds available at the facility' },
    { key: 'EMERGENCY_CONTACT_1', value: '061-9200450 (Sweet Home Control Room)', group: 'SECURITY', description: '24/7 Control Helpline' },
    { key: 'TIMEZONE', value: 'Asia/Karachi', group: 'INSTITUTION', description: 'Timezone used for daily scheduled notifications' },
    { key: 'SMS_ENABLED', value: 'false', group: 'NOTIFICATIONS', description: 'Enable outbound SMS notifications' },
    { key: 'WEEKLY_OFF_SMS_ENABLED', value: 'false', group: 'NOTIFICATIONS', description: 'Enable daily weekly-off SMS notifications' },
    { key: 'WEEKLY_OFF_SMS_TEMPLATE', value: 'Dear {employeeName}, today is your scheduled weekly off. Your next reporting day is {nextReportingDay}. - PBM Sweet Home Multan', group: 'NOTIFICATIONS', description: 'Weekly-off SMS template' },
  ];
