...
Set `DATABASE_URL` in Vercel Production to the existing Supabase PostgreSQL connection string. The production build applies only pending migrations:
```bash
npx prisma migrate deploy
```
Do not use `prisma db push` or `prisma migrate reset` in production. Run the seed only as a deliberate, separate operation when required.
...
3. Set Build Command: `npm run build` (the repository build script runs `prisma migrate deploy`, then `prisma generate`, then `next build`).
4. Click **Deploy**.
...
### Weekly-Off SMS Gateway

Weekly-off SMS uses a provider-agnostic server-side HTTP gateway. The gateway can be an Android SMS gateway device or another compatible service; no vendor is hardcoded in the ERP.

Required server-only variables:

```env
SMS_GATEWAY_ENABLED=false
SMS_GATEWAY_URL=https://gateway.example/send
SMS_GATEWAY_API_KEY=server-only-key
SMS_GATEWAY_DEVICE_ID=dedicated-device-id
SMS_GATEWAY_TIMEOUT_MS=15000
```

The gateway receives JSON with `to`, `message`, `externalReference`, and optionally `deviceId`, and should return JSON containing `providerMessageId` or `id` on success. Use `GET /api/cron/weekly-off?dryRun=1` with the existing `Authorization: Bearer $CRON_SECRET` header to preview eligible messages. Dry-run mode never sends SMS or records `SENT`.

Enable both `SMS_ENABLED` and `WEEKLY_OFF_SMS_ENABLED` in administrative settings, then set `SMS_GATEWAY_ENABLED=true` only after the dedicated SIM device and HTTPS gateway are ready. Each notification uses `WEEKLY_OFF:{employeeId}:{date}` as its external reference and is protected by the existing unique SMS log key. Failed attempts are recorded as `FAILED`; successful sends are recorded as `SENT` with provider ID and sent time.
